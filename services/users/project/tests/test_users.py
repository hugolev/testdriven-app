...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        self.assertIn('hugo@le.com was added!', data['message'])
        self.assertIn('success', data['status'])
    ...
    def test_add_user_invalid_json_keys(self):
        """Ensure error thrown if the JSON object doesn't have a uname key."""
        with self.client:
            response = self.client.post(
                    '/users',
                    data=json.dumps({'email': 'hugo@le.com'}),
                    content_type='application/json',
                    )
            data = json.loads(response.data.decode())
        self.assertEqual(response.status_code, 400)
    ...
    def test_single_user(self):
        ...
        user = add_user('hugo', 'hugo@le.com')
        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('hugo', data['data']['username'])
            self.assertIn('hugo@le.com', data['data']['email'])
            self.assertIn('success', data['status'])
